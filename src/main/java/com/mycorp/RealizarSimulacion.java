...
public class RealizarSimulacion {
    ...
    public Map< String, Object > realizarSimulacion(final DatosAlta oDatosAlta,
        ...
        Set< FrecuenciaEnum > frecuenciasTarificar = obtenerFrecuencias(oDatosAlta,lBeneficiarios,hmValores);
        ...
        obtenerResultadoSimulacion(frecuenciasTarificar,
                oDatosAlta,lProductos,
                lBeneficiarios, hmValores,
                resultadoSimulaciones, resultadoExcepciones  );
        ...
        for( final FrecuenciaEnum frecuencia : frecuenciasTarificar ) {
            ...
            if(!StringUtils.isEmpty( codigoError ) ) {
            ...
            for( final TarifaBeneficiario tarifaBeneficiario : retorno.getTarifas().getTarifaBeneficiarios() ) {
                ...
                for( final TarifaProducto tarifaProducto : tarifaBeneficiario.getTarifasProductos() ) {
                    css = calcularDescuentos(tarifaProducto,  frecuencia,
                            oDatosAlta, oDatosPlan,
                            lExcepciones, listaProductoPorAseg,
                            css, contadorProducto,
                            descuentosTotales, pagoTotal);
                    contadorProducto++;
                }
            ...
            if( retorno.getRecibos() != null ) {
                ...
                precioConPromocion[ frecuencia.getValor() - 1 ] =
                        retorno.getRecibos().getReciboPoliza().getRecibos()[ 0 ].getImporte() - css;
            }
        }
    ...
    private Set< FrecuenciaEnum > obtenerFrecuencias(final DatosAlta oDatosAlta,
                                                     final List< BeneficiarioPolizas > lBeneficiarios,
                                                     final Map< String, Object > hmValores) {
    ...
    private void obtenerResultadoSimulacion(Set< FrecuenciaEnum > frecuenciasTarificar,
                                            final DatosAlta oDatosAlta,
                                            final List< ProductoPolizas > lProductos,
                                            final List< BeneficiarioPolizas > lBeneficiarios,
                                            final Map< String, Object > hmValores,
                                            List< TarificacionPoliza > resultadoSimulaciones,
                                            List< ExecutionException > resultadoExcepciones  ) {
        final CompletionService< TarificacionPoliza > ecs = new ExecutorCompletionService< >( pool );
        List<Future<TarificacionPoliza>> futures = new ArrayList<>();
        Callable< TarificacionPoliza > s = null;
        for( final FrecuenciaEnum frecuencia : frecuenciasTarificar ) {
            try {
                s = simularPolizaFrecuencia( hmValores, oDatosAlta, lProductos, lBeneficiarios, frecuencia );
                futures.add(ecs.submit(s));
            } catch( final RuntimeException ree ) {
                if (s != null) {
                    LOG.error("RejectedExecutionException con el metodo " + s.toString(), ree);
                } else {
                    LOG.error("RejectedExecutionException con el metodo ", ree);
                }
            }
        }

        for (Future<TarificacionPoliza> future : futures) {
            try {
                resultadoSimulaciones.add( future.get(TIMEOUT, TimeUnit.SECONDS) );
    ...
    private double calcularDescuentos(final TarifaProducto tarifaProducto,
        ...
        if ((tarifaProducto.getIdProducto() != 389
            ...
            cssCalculado = css + tarifaDesglosada.getCss();
    ...
    private TarificacionPoliza simular( final Map< String, Object > hmValores, final DatosAlta oDatosAlta,
        ...
        in.setOperacion(
                lBeneficiarios != null
                        ...
                        :  StaticVarsContratacion.ALTA_POLIZA
        );
        ...
        in.setInfoContratacion(
                obtenerInfoContratacion( oDatosAlta, frecuencia, in.getOperacion())
        );
     ...
     *
     * @param  promocionesAplicadas múltiple realizada
     */
    ...
    private String cambiarFecha( String fecha ) {
        ...
        if( !fecha.contains( "/" ) ) {
            int edad = Integer.valueOf( fecha );
    ...
    private boolean hayTarjetas( final DatosAlta oDatosAlta ) {
        ...
        if( oDatosAlta != null &&
                oDatosAlta.getTitular() != null &&
                "S".equals( oDatosAlta.getTitular().getSwPolizaAnterior() )
                ) {
            tieneTarjeta = true;
        }
        if( oDatosAlta != null &&
                oDatosAlta.getAsegurados() != null &&
